...
import { CODIGOS_ASISTENCIA, calcularGrupo, DIAS_SEMANA } from "@/lib/planificacion/cuadrillas";
...
export default function PlanDetalleePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [tab, setTab] = useState<"seleccion" | "programacion" | "ots" | "roster">("seleccion");
  const [filtroOts, setFiltroOts] = useState("");
  const [filtroGrupoOts, setFiltroGrupoOts] = useState("");
  // Dentro de la pestaña Programación, el planificador elige entre el Tablero
  // (kanban, drag&drop) y la Lista (tipo planilla, igual patrón que el
  // toggle Lista/Tablero de Reporte de OT) — mismos datos, dos formas de
  // llenarlos según la costumbre de cada planificador.
  const [vistaProgramacion, setVistaProgramacion] = useState<"tablero" | "lista">("tablero");
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f1f5f9" }}>
      ...
      <main style={{ maxWidth: tab === "programacion" ? "none" : 1100, margin: "0 auto", padding: "20px 16px" }}>
        {/* Header del plan */}
        ...
        <div style={{ display: "flex", gap: 2, marginBottom: 14 }}>
          {(["seleccion", "programacion", "ots", "roster"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)} style={{
              ...
              {t === "seleccion" ? `✅ Selección (${plan.ots.filter(o => o.seleccionada).length})`
                : t === "programacion" ? `📌 Programación (${plan.ots.length})`
          ...
          <div style={{ background: "white", borderRadius: "0 12px 12px 12px", padding: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
            ...
            {plan.ots.length === 0 ? (
              <div style={{ padding: 40, textAlign: "center", color: "#94a3b8" }}>
                No hay OTs en este plan todavía. Importa el Excel JDE para empezar a armar la semana.
              </div>
            ) : (
        ...
        {/* Tab Programación: Tablero (kanban) o Lista (planilla) — mismos datos */}
        {tab === "programacion" && (
          <>
            <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
              {(["lista", "tablero"] as const).map(m => (
                <button
                  key={m}
                  onClick={() => setVistaProgramacion(m)}
                  style={{
                    padding: "6px 16px", borderRadius: 20, border: "none", cursor: "pointer",
                    fontSize: 12, fontWeight: 700,
                    background: vistaProgramacion === m ? "#7c3aed" : "#e2e8f0",
                    color: vistaProgramacion === m ? "white" : "#64748b",
                  }}
                >
          ...
          <div style={{ background: "white", borderRadius: "0 12px 12px 12px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
            <ListaPrograma
              plan={plan}
              cuadrilla={cuadrilla}
              onPatchOt={patchOtLocal}
        ...
        {/* Tab OTs: lista completa de OTs importadas, editable/eliminable fila por fila */}
        {tab === "ots" && (
                  ...
                  return (
                    <div style={{ overflowX: "auto" }}>
                      <table style={{ width: "100%", borderCollapse: "collapse" }}>
                        ...
                        <tbody>
                          {otsFiltradas.map(ot => (
                            <OtRow key={ot.id} ot={ot} onSave={patchOtLocal} onDelete={deleteOt} />
                          ))}
                        </tbody>
                      </table>
                    </div>
